...
MyString::MyString()//Конструктор стандартный 
{
	str = nullptr;
}

MyString::MyString(const char* str) //Конструктор для текста
{
...
MyString::~MyString() //Диструктор
{
	delete[] this->str;
}

MyString& MyString::operator =(const MyString& other) //Перегрузка оператора =
{
	if (this->str != nullptr)
	{
		delete[] str;
...
bool MyString::operator==(const MyString& other) //Перегрузка оператора ==
{
...
char& MyString::operator [] (const int index) //Перегрузка оператора []
{
